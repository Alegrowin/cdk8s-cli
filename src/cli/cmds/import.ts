...
class Command implements yargs.CommandModule {
  ...
  public readonly builder = (args: yargs.Argv) => args
    .positional('SPEC', { default: config?.imports, desc: 'import spec with the syntax [NAME:=]SPEC where NAME is an optional module name and supported SPEC are: k8s, crd.yaml, k8s-manifest:deployment.yaml, https://domain/crd.yaml, github:account/repo[@VERSION]).', array: true })
    ...
    .example('cdk8s import deployment.yaml', 'Imports Kubernetes manifests from a YAML file')
    .example('cdk8s import k8s-manifest:deployment.yaml', 'Explicitly imports Kubernetes manifests from a YAML file')
    .example('cdk8s import k8s-manifest:manifests/', 'Imports all Kubernetes manifests from a directory')
    ...
    .option('k8s-api-version', { type: 'string', default: DEFAULT_API_VERSION, desc: 'The Kubernetes API version to use for manifest imports. This should match the version used in the k8s import.' })
    ...
    .option('unsupported', { type: 'boolean', default: false, desc: 'Allow generation of code for unsupported APIs (Typescript Only). When disabled, unsupported resources will be logged' });
  ...
  public async handler(argv: any) {
    ...
    // Convert kebab-case to camelCase for the k8s-api-version option
    argv.k8sApiVersion = argv['k8s-api-version'];
